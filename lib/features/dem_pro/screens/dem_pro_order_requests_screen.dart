...
// Informatif seulement — comment le client final compte payer à la
// livraison (espèces/Wave/Orange Money/Free Money), jamais un encaissement
// en ligne réel (voir public.service.js:submitOrderRequest).
...
class DemProOrderRequestsScreen extends StatefulWidget {
  ...
  @override
  State<DemProOrderRequestsScreen> createState() =>
      _DemProOrderRequestsScreenState();
}
...
class _DemProOrderRequestsScreenState extends State<DemProOrderRequestsScreen> {
  ...
  Future<void> _confirm(Map<String, dynamic> request) async {
    await context.push(
      '/dem-pro/orders/create',
      extra: {'fromOrderRequest': request},
    );
    if (mounted) _load();
  }
...
class _RequestCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final name = request['customerName'] as String? ?? 'Client';
    ...
    final total = (request['total'] as num?) ?? 0;
    final paymentMethod = request['customerPaymentMethod'] as String?;
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      _timeAgo(request['createdAt'] as String?),
                      style: ClientText.label.copyWith(
                        color: AppColors.textMuted,
                      ),
                    ),
