...
class _State extends State<ChefDeFlotteAddDriverScreen> {
  ...
  int get _docsUploaded =>
    [_licenseFront, _licenseBack, _vehiclePhoto, _carteGrise, _assurance, _casquePhoto]
              ...
              child: Column(children: [
                Padding(
                  ...
                  child: Row(children: [
                    ...
                    Text('${ _step + 1}/$_total',
                      style: TextStyle(color: Colors.white.withValues(alpha: 0.70), fontSize: 13, fontWeight: FontWeight.w700)),
                  ]),
                ),
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Column(children: [
      ...
      Row(
        ...
        children: List.generate(total, (i) => _StepDot(
          index:    i,
          current:  current,
          ...
          child: done
              ...
              : Text(
                  ...
                  style: TextStyle(
                    ...
                    color: active ? AppColors.primaryDark : Colors.white.withValues(alpha: 0.55),
                  ),
                ),
        ),
      ),
...
class _NavBar extends StatelessWidget {
  ...
  const _NavBar({required this.step, required this.total, required this.loading,
    required this.canNext, required this.nextLabel, required this.onNext, this.onPrev});
      ...
      if (onPrev != null)
        Expanded(
          ...
          child: OutlinedButton(
            ...
            child: const Text('Précédent', style: TextStyle(fontWeight: FontWeight.w700)),
          ),
        ),
              ...
              child: Center(
                child: loading
                    ...
                    : Text(nextLabel, style: TextStyle(
                        color: canNext ? Colors.white : Colors.grey.shade400,
...
class _Step1 extends StatelessWidget {
  ...
  const _Step1({required this.nameCtrl, required this.phoneCtrl,
    required this.nameError, required this.digitCount});

  @override
  Widget build(BuildContext context) => SingleChildScrollView(
    padding: const EdgeInsets.fromLTRB(20, 28, 20, 20),
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      _StepHeading(icon: Icons.person_outline, title: 'Qui est ce livreur ?',
      ...
      Row(children: [
        Container(
          ...
          child: const Text('+221', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 14, color: Color(0xFF374151))),
        ),
      ...
      if (digitCount > 0 && digitCount < 8)
        Padding(
          ...
          child: Text('Numéro incomplet (${9 - digitCount} chiffre(s) manquant(s))',
...
class _Step2 extends StatelessWidget {
  final void Function(String?) onLicenseFront, onLicenseBack,
      onVehiclePhoto, onCarteGrise, onAssurance, onCasquePhoto;
  final DateTime? insuranceExpiry;
  final VoidCallback onPickDate;
  const _Step2({
    required this.onLicenseFront,  required this.onLicenseBack,
    required this.onVehiclePhoto,  required this.onCarteGrise,
    required this.onAssurance,     required this.onCasquePhoto,
    required this.insuranceExpiry, required this.onPickDate,
  });
  ...
  Widget build(BuildContext context) => SingleChildScrollView(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      _DocSection(
        ...
        children: [
          DocPickerField(label: 'Permis recto', fieldKey: 'licenseFront', onChanged: onLicenseFront),
          const SizedBox(height: 10),
          DocPickerField(label: 'Permis verso',  fieldKey: 'licenseBack',  onChanged: onLicenseBack),
        ],
      ),
      ...
      _DocSection(
        ...
        children: [
          DocPickerField(label: 'Photo moto (plaque visible)', fieldKey: 'vehiclePhoto', onChanged: onVehiclePhoto),
          const SizedBox(height: 10),
          DocPickerField(label: 'Carte grise',                  fieldKey: 'carteGrise',   onChanged: onCarteGrise),
          const SizedBox(height: 10),
          DocPickerField(label: "Attestation d'assurance",      fieldKey: 'assurance',    onChanged: onAssurance),
          const SizedBox(height: 10),
            ...
            child: Container(
              ...
              child: Row(children: [
                Icon(Icons.calendar_today_outlined, size: 18,
                  color: insuranceExpiry != null ? Colors.green.shade600 : const Color(0xFF9CA3AF)),
                ...
                Expanded(
                  child: Text(
                    ...
                    style: TextStyle(
                      fontSize: 14,
                      color: insuranceExpiry != null ? const Color(0xFF065F46) : const Color(0xFF9CA3AF),
                      fontWeight: insuranceExpiry != null ? FontWeight.w600 : FontWeight.w400,
                    ),
                  ),
                ),
      ...
      _DocSection(
        ...
        children: [
          DocPickerField(label: 'Photo casque', fieldKey: 'casquePhoto', onChanged: onCasquePhoto),
        ],
      ),
    ]),
  );
...
class _Step3 extends StatelessWidget {
  ...
  const _Step3({
    ...
    required this.docsUploaded, required this.docsTotal,
    required this.docs, required this.insuranceExpiry, required this.error,
  });
  ...
  Widget build(BuildContext context) => SingleChildScrollView(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      // Infos livreur
      _RecapCard(children: [
        _RecapRow(icon: Icons.person_outline, label: 'Nom', value: name.isEmpty ? '—' : name),
        _RecapRow(icon: Icons.phone_outlined,  label: 'Téléphone', value: phone),
      ]),
      const SizedBox(height: 12),
      ...
      _RecapCard(children: [
        Padding(
          ...
          child: Row(children: [
            const Text('Documents', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 13, color: Color(0xFF374151))),
            const Spacer(),
            Container(
              ...
              child: Text('$docsUploaded/$docsTotal ajoutés',
                style: TextStyle(
                  fontSize: 11, fontWeight: FontWeight.w700,
                  color: docsUploaded == docsTotal ? Colors.green.shade700 : AppColors.primaryMid,
                )),
            ),
          ]),
        ),
        ...docs.entries.map((e) => Padding(
          padding: const EdgeInsets.only(bottom: 6),
          child: Row(children: [
            Icon(
              e.value != null ? Icons.check_circle_outline : Icons.radio_button_unchecked,
              size: 16,
              color: e.value != null ? Colors.green.shade600 : Colors.grey.shade400,
            ),
            const SizedBox(width: 8),
            Text(e.key, style: TextStyle(
              fontSize: 13,
              color: e.value != null ? const Color(0xFF1F2937) : Colors.grey.shade400,
            )),
          ]),
        )),
        if (insuranceExpiry != null)
          Padding(
            ...
            child: Row(children: [
              Icon(Icons.calendar_today_outlined, size: 15, color: Colors.green.shade600),
              const SizedBox(width: 8),
              Text(
...
class _StepHeading extends StatelessWidget {
  ...
  const _StepHeading({required this.icon, required this.title, required this.subtitle});
  @override
  Widget build(BuildContext context) => Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Container(
      width: 44, height: 44,
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [AppColors.primary.withValues(alpha: 0.15), AppColors.primaryMid.withValues(alpha: 0.08)]),
        borderRadius: BorderRadius.circular(12),
      ),
    ...
    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(title, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: Color(0xFF0F2942))),
...
class _DocSection extends StatelessWidget {
  ...
  const _DocSection({required this.icon, required this.title, required this.color, required this.children});
  @override
...
class _RecapRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Row(children: [
      ...
      Expanded(child: Text(value, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: Color(0xFF1F2937)))),
    ]),
  );
}

InputDecoration _inputDec(String hint, IconData icon, {String? error}) => InputDecoration(
  hintText: hint,
